#!/usr/bin/env python3
"""
RT-DETR Training Script with MLflow Integration

A script to train RT-DETR on MTMMC dataset using Ultralytics with proper MLflow tracking.
Follows the same integration pattern as FasterRCNN training.
...
try:
    ...
    print(f"Error importing local modules in train_rtdetr_simple.py: {e}")
...
def run_rtdetr_training(run_config: Dict[str, Any], device: str) -> str:
    """Execute RT-DETR training within MLflow context"""
    active_run = mlflow.active_run()
    if not active_run:
        logger.critical("run_rtdetr_training called without an active MLflow run!")
        return "FAILED"
...
def main():
    """Main execution function with MLflow integration"""
    logger.info("--- Starting RT-DETR Training Run ---")
    config_path_str = "configs/rtdetr_training_config.yaml"
    final_status = "FAILED"
    run_id = None
    
    # Try to load config, fallback to default if file doesn't exist
    config = {}
    try:
        config = load_config(config_path_str)
        logger.info(f"Loaded configuration from {config_path_str}")
    except Exception as e:
        logger.warning(f"Could not load config file {config_path_str}: {e}")
        logger.info("Using default configuration")
    
    # Create default config structure if not loaded
    if not config:
        config = {
            "environment": {
                "seed": int(time.time()),
            ...
            "mlflow": {
                "experiment_name": "RT-DETR Training"
            },
            "model": {
                "type": "rtdetr",
                "name_tag": "rtdetr_simple"
            },
            "training": {
            ...
            "run_name": "RT-DETR Simple Training"
        }
    
    # Setup MLflow experiment
    experiment_id = setup_mlflow_experiment(
        config, 
        default_experiment_name="RT-DETR Training"
    )
    ...
    try:
        # Start MLflow run
        run_name = config.get("run_name", "RT-DETR Training")
        with mlflow.start_run(run_name=run_name, experiment_id=experiment_id) as run:
            ...
            # Execute training
            final_status = run_rtdetr_training(config, str(resolved_device))

"""
YOLO12-N Training Runner with Local Results Storage

This script runs YOLO12-N model training using the Ultralytics framework,
following the same structure as the RT-DETR training runner.

Usage:
    python src/run_training_yolo12.py

The script will:
1. Load configuration from configs/yolo12_training_config.yaml
2. Train YOLO12-N model using Ultralytics with person detection focus
...
try:
    ...
    print(f"Error importing local modules in run_training_yolo12.py: {e}")
    print("Please ensure all modules exist and PYTHONPATH is set correctly.")
    sys.exit(1)

# --- Basic Logging Setup ---
log_file = setup_logging(log_prefix="train_yolo12", log_dir=PROJECT_ROOT / "logs")
...
def create_coco_format_data(run_config: Dict[str, Any], output_dir: Path):
    """Convert MTMMC data to COCO format for YOLO12 training - Environment-aware with camera filtering"""
...
def create_dataset_yaml(output_dir: Path):
    """Create dataset YAML file for YOLO12"""
...
def run_yolo12_training_job(run_config: Dict[str, Any], device: str, project_root: Path) -> tuple[str, Optional[Dict[str, Any]]]:
    """Execute YOLO12-N training with MLflow logging"""
    active_run = mlflow.active_run()
    if not active_run:
        logger.critical("run_yolo12_training_job called without an active MLflow run!")
    ...
    logger.info(f"--- Starting YOLO12-N Training Job (Run ID: {run_id}) ---")
    ...
    # Output directory
    output_dir = project_root / "yolo12_training_output"
    ...
    try:
        ...
        mlflow.set_tag("model_type", "yolo12")
        ...
        if pretrained_weights and Path(pretrained_weights).exists():
            ...
            model = YOLO(model_size)  # Load default pretrained YOLO12-N model
        ...
        training_params = {
            ...
            'name': training_config.get('name', 'yolo12_experiment'),
        ...
        logger.info("Starting YOLO12-N training with MLflow logging...")
        ...
        logger.info(f"--- YOLO12-N Training Finished in {training_duration:.2f} seconds ---")
        ...
        experiment_name = training_config.get('name', 'yolo12_experiment')
        experiment_dir = output_dir / experiment_name
        checkpoint_dir = project_root / "checkpoints" / "yolo12" / run_id
        ...
        if experiment_dir.exists():
            logger.info("Experiment directory found! Logging comprehensive YOLO12-N training artifacts...")
            ...
            logger.info(f"All YOLO12-N training artifacts logged successfully! ({artifact_count} files)")
        else:
        ...
        logger.info("YOLO12-N training completed successfully!")
...
def main():
    """
    Runs a single YOLO12-N training job based on the configuration file.
    
    Usage:
        python src/run_training_yolo12.py [config_path]
    
    Examples:
        python src/run_training_yolo12.py configs/yolo12_factory_config.yaml
        python src/run_training_yolo12.py configs/yolo12_campus_config.yaml
        python src/run_training_yolo12.py  # Uses default config
    """
    logger.info("--- Starting YOLO12-N Training Run ---")
    ...
    if len(sys.argv) > 1:
        ...
        config_path_str = "configs/yolo12_training_config.yaml"
        logger.info("Using default config file (consider using environment-specific configs)")
        logger.info("Available configs: yolo12_factory_config.yaml, yolo12_campus_config.yaml")
    ...
    if not config:
        ...
        logger.info("  - configs/yolo12_factory_config.yaml (Factory scenes)")
        logger.info("  - configs/yolo12_campus_config.yaml (Campus scenes)")
        sys.exit(1)
    ...
    # --- Construct the config needed by run_yolo12_training_job ---
    single_run_config = {
        ...
        "run_name": config.get(
            "run_name", job_config_entry["model"].get("name_tag", "yolo12_run")
        ),
    }

    # 2. Setup MLflow Experiment
    experiment_id = setup_mlflow_experiment(
        config, default_experiment_name="Default YOLO12-N Training"
    )
    ...
    try:
        ...
        logger.info("--- Starting YOLO12-N Training ---")
        ...
        with mlflow.start_run(run_name=run_name, experiment_id=experiment_id) as active_run:
            ...
            run_status, final_metrics = run_yolo12_training_job(
    ...
    logger.info(f"--- YOLO12-N Training Run Completed (Status: {final_status}) ---")

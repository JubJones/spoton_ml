# ================================================
# FILE: src/run_training_fasterrcnn.py (RENAMED & MODIFIED)
# ================================================
import logging
import sys
import time
...
try:
    ...
    # Import core runner helpers only if needed (like log_git_info - still needed by runner)
    # from src.core.runner import log_git_info # Not directly called here anymore
...
def main():
    ...
    if not models_to_train or not isinstance(models_to_train, list) or len(models_to_train) == 0:
        logger.critical(f"Config {config_path_str} must contain a list 'models_to_train' with at least one entry.")
        sys.exit(1)
    if len(models_to_train) > 1:
        logger.warning(f"Config contains {len(models_to_train)} entries in 'models_to_train'. Using only the first one.")

    job_config_entry = models_to_train[0]
    if "model" not in job_config_entry or "training" not in job_config_entry:
        logger.critical("The first entry in 'models_to_train' must contain 'model' and 'training' keys.")
        sys.exit(1)
    ...
    single_run_config = {
        ...
        "mlflow": config.get("mlflow", {}), # Pass mlflow config for experiment name etc.
        ...
        "run_name": config.get("run_name", job_config_entry["model"].get("name_tag", "fasterrcnn_run"))
    }
    # ------------------------------------------------------------

    # 2. Setup MLflow Experiment
    experiment_id = setup_mlflow_experiment(config, default_experiment_name="Default FasterRCNN Training")
    ...
    try:
        ...
        run_name = single_run_config["run_name"] # Get run name from constructed config
        with mlflow.start_run(run_name=run_name, experiment_id=experiment_id) as run:
            ...
            run_status, _ = run_single_training_job(
                run_config=single_run_config, # Pass the extracted & constructed config
        ...
        if run_id and log_file.exists():
            try:
                # Ensure logs are flushed before attempting to log
                for handler in logging.getLogger().handlers: handler.flush()
                client = MlflowClient()
                client.log_artifact(run_id, str(log_file), artifact_path="logs")
                logger.info(f"Main training log file '{log_file.name}' logged as artifact to run {run_id}.")
            except Exception as log_artifact_err:
                logger.warning(f"Could not log main training log file artifact '{log_file}': {log_artifact_err}")
        ...
        elif run_id: # If run exists but isn't active (e.g., due to error)
            try:

"""
YOLO11-L Training Runner with Local Results Storage

This script runs YOLO11-L model training using the Ultralytics framework,
following the same structure as the RT-DETR training runner.

Usage:
    python src/run_training_yolo11.py

The script will:
1. Load configuration from configs/yolo11_training_config.yaml
2. Train YOLO11-L model using Ultralytics with person detection focus
...
try:
    ...
    print(f"Error importing local modules in run_training_yolo11.py: {e}")
    print("Please ensure all modules exist and PYTHONPATH is set correctly.")
    sys.exit(1)

# --- Basic Logging Setup ---
log_file = setup_logging(log_prefix="train_yolo11", log_dir=PROJECT_ROOT / "logs")
...
def create_coco_format_data(run_config: Dict[str, Any], output_dir: Path):
    """Convert MTMMC data to COCO format for YOLO11 training - Environment-aware with camera filtering"""
...
def create_dataset_yaml(output_dir: Path):
    """Create dataset YAML file for YOLO11"""
...
def run_yolo11_training_job(run_config: Dict[str, Any], device: str, project_root: Path) -> tuple[str, Optional[Dict[str, Any]]]:
    """Execute YOLO11-L training with MLflow logging"""
    active_run = mlflow.active_run()
    if not active_run:
        logger.critical("run_yolo11_training_job called without an active MLflow run!")
    ...
    logger.info(f"--- Starting YOLO11-L Training Job (Run ID: {run_id}) ---")
    ...
    # Output directory
    output_dir = project_root / "yolo11_training_output"
    ...
    try:
        ...
        mlflow.set_tag("model_type", "yolo11")
        ...
        if pretrained_weights and Path(pretrained_weights).exists():
            ...
            model = YOLO(model_size)  # Load default pretrained YOLO11-L model
        ...
        training_params = {
            ...
            'name': training_config.get('name', 'yolo11_experiment'),
        ...
        logger.info("Starting YOLO11-L training with MLflow logging...")
        ...
        logger.info(f"--- YOLO11-L Training Finished in {training_duration:.2f} seconds ---")
        ...
        experiment_name = training_config.get('name', 'yolo11_experiment')
        experiment_dir = output_dir / experiment_name
        checkpoint_dir = project_root / "checkpoints" / "yolo11" / run_id
        ...
        if experiment_dir.exists():
            logger.info("Experiment directory found! Logging comprehensive YOLO11-L training artifacts...")
            ...
            logger.info(f"All YOLO11-L training artifacts logged successfully! ({artifact_count} files)")
        else:
        ...
        logger.info("YOLO11-L training completed successfully!")
...
def main():
    """
    Runs a single YOLO11-L training job based on the configuration file.
    
    Usage:
        python src/run_training_yolo11.py [config_path]
    
    Examples:
        python src/run_training_yolo11.py configs/yolo11_factory_config.yaml
        python src/run_training_yolo11.py configs/yolo11_campus_config.yaml
        python src/run_training_yolo11.py  # Uses default config
    """
    logger.info("--- Starting YOLO11-L Training Run ---")
    ...
    if len(sys.argv) > 1:
        ...
        config_path_str = "configs/yolo11_training_config.yaml"
        logger.info("Using default config file (consider using environment-specific configs)")
        logger.info("Available configs: yolo11_factory_config.yaml, yolo11_campus_config.yaml")
    ...
    if not config:
        ...
        logger.info("  - configs/yolo11_factory_config.yaml (Factory scenes)")
        logger.info("  - configs/yolo11_campus_config.yaml (Campus scenes)")
        sys.exit(1)
    ...
    # --- Construct the config needed by run_yolo11_training_job ---
    single_run_config = {
        ...
        "run_name": config.get(
            "run_name", job_config_entry["model"].get("name_tag", "yolo11_run")
        ),
    }

    # 2. Setup MLflow Experiment
    experiment_id = setup_mlflow_experiment(
        config, default_experiment_name="Default YOLO11-L Training"
    )
    ...
    try:
        ...
        logger.info("--- Starting YOLO11-L Training ---")
        ...
        with mlflow.start_run(run_name=run_name, experiment_id=experiment_id) as active_run:
            ...
            run_status, final_metrics = run_yolo11_training_job(
    ...
    logger.info(f"--- YOLO11-L Training Run Completed (Status: {final_status}) ---")

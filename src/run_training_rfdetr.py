...
try:
    ...
    from src.components.training.rfdetr_runner import run_single_rfdetr_training_job
except ImportError as e:
    print(f"Error importing local modules in run_training_rfdetr.py: {e}")
    print("Please ensure all modules exist and PYTHONPATH is set correctly.")
    sys.exit(1)

# --- Basic Logging Setup ---
log_file = setup_logging(log_prefix="train_rfdetr", log_dir=PROJECT_ROOT / "logs")
...
def main():
    """
    Runs a single RF-DETR training job based on the configuration file.
    """
    logger.info("--- Starting RF-DETR Training Run ---")
    config_path_str = "configs/rfdetr_training_config.yaml"
    final_status = "FAILED"
    run_id = None
    ...
    # --- Construct the config needed by run_single_rfdetr_training_job ---
    single_run_config = {
        ...
        "run_name": config.get(
            "run_name", job_config_entry["model"].get("name_tag", "rfdetr_run")
        ),
    }

    # 2. Setup MLflow Experiment
    experiment_id = setup_mlflow_experiment(
        config, default_experiment_name="Default RF-DETR Training"
    )
    ...
    try:
        ...
        with mlflow.start_run(run_name=run_name, experiment_id=experiment_id) as run:
            ...
            run_status, _ = run_single_rfdetr_training_job(
    ...
    logger.info(f"--- RF-DETR Training Run Completed (Status: {final_status}) ---")
